        ...
        <div style={{ padding: 24 }}>
          {!driver ? (
            ...
            <div>
              ...
              {error && <div style={{ background: '#450a0a', border: '1px solid #991b1b', color: '#fca5a5', padding: '10px 14px', borderRadius: 8, fontSize: 13, marginBottom: 16 }}>⚠️ {error}</div>}
              {info && <div style={{ background: '#172554', border: '1px solid #1e40af', color: '#93c5fd', padding: '10px 14px', borderRadius: 8, fontSize: 13, marginBottom: 16 }}>ℹ️ {info}</div>}
              ...
              {!otpSent ? (
                <form onSubmit={handleSendOtp} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  <button type="submit" disabled={loading} style={{ padding: '12px', borderRadius: 8, border: 'none', background: '#dc2626', color: '#fff', fontWeight: 700, fontSize: 15, cursor: 'pointer' }}>
                    {loading ? 'Sending OTP...' : '📲 Send Driver OTP'}
                  </button>
                </form>
              ) : (
                <form onSubmit={handleVerifyOtp} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  <button type="submit" disabled={loading} style={{ padding: '12px', borderRadius: 8, border: 'none', background: '#16a34a', color: '#fff', fontWeight: 700, fontSize: 15, cursor: 'pointer' }}>
                    {loading ? 'Verifying...' : '✅ Confirm Driver Sign In'}
                  </button>
                </form>
              )}

              <div style={{ marginTop: 20, padding: 12, borderRadius: 8, background: '#1e293b', fontSize: 12, color: '#94a3b8' }}>
                💡 <b>Demo Driver Account:</b> <code>+919876543214</code> (OTP: <code>123456</code>)
              </div>
            </div>
          ) : (
            /* DRIVER DASHBOARD */
              ...
              {activeCall ? (
                <div style={{ background: '#1e1b4b', border: '1px solid #4338ca', borderRadius: 12, padding: 16, marginBottom: 20 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                    <span style={{ background: '#dc2626', color: '#fff', padding: '3px 8px', borderRadius: 4, fontSize: 11, fontWeight: 800, textTransform: 'uppercase' }}>
                      🔥 {activeCall.priority} Dispatch
                    </span>
                  ...
                  <div style={{ fontSize: 13, color: '#cbd5e1', marginBottom: 4 }}>📍 Pickup: <b>{activeCall.pickup_location?.address || 'Gandhipuram Signal'}</b></div>
                  <div style={{ fontSize: 13, color: '#cbd5e1', marginBottom: 4 }}>🏥 Hospital: <b>{activeCall.destination_hospital}</b></div>
                  <div style={{ fontSize: 13, color: '#cbd5e1', marginBottom: 12 }}>📞 Phone: <a href={`tel:${activeCall.contact_phone}`} style={{ color: '#38bdf8' }}>{activeCall.contact_phone}</a></div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    <button onClick={() => handleUpdateStatus('en_route')} disabled={updating} style={{ padding: '10px', background: activeCall.status === 'dispatched' ? '#2563eb' : '#1e293b', border: '1px solid #3b82f6', color: '#fff', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>
                      🚑 En Route
                    </button>
                    <button onClick={() => handleUpdateStatus('on_scene')} disabled={updating} style={{ padding: '10px', background: activeCall.status === 'on_scene' ? '#d97706' : '#1e293b', border: '1px solid #f59e0b', color: '#fff', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>
                      📍 On Scene
                    </button>
                    <button onClick={() => handleUpdateStatus('transporting')} disabled={updating} style={{ padding: '10px', background: activeCall.status === 'transporting' ? '#9333ea' : '#1e293b', border: '1px solid #a855f7', color: '#fff', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>
                      🏥 Transporting
                    </button>
                    <button onClick={() => handleUpdateStatus('available')} disabled={updating} style={{ padding: '10px', background: '#16a34a', border: 'none', color: '#fff', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>
                      ✅ Call Completed
                    </button>
                  </div>
                </div>
