...
export default function DriverMap({
  ...
  useEffect(() => {
    if (driverRole === 'ambulance_driver' && activeDispatch && driverLocation) {
      const patientCoords = activeDispatch.pickup_location?.coordinates; // [lng, lat]
      if (patientCoords) {
        const url = `https://router.project-osrm.org/route/v1/driving/${driverLocation[0]},${driverLocation[1]};${patientCoords[0]},${patientCoords[1]}?overview=full&geometries=geojson`;
        fetch(url)
          .then(res => res.json())
          .then(data => {
